      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          ...
          <StatCard
            ...
            sub={
              stats ? `${formatCurrency(stats.revenueThisWeek)} this week` : ""
            }
            color="emerald"
          />
          <StatCard
        ...
        {stats && (
          <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <MiniStat
              ...
              value={stats.totalCreditsRemaining.toString()}
            />
            <MiniStat
              label="Credits Sold"
              value={stats.totalCreditsSold.toString()}
            />
          </div>
        )}
            ...
            {stats && stats.recentPurchases.length > 0 ? (
              <div className="space-y-2">
                {stats.recentPurchases.slice(0, 8).map((p) => (
                  ...
                  >
                    <div>
                      <p className="text-muted-foreground max-w-[180px] truncate font-mono text-xs">
                        {p.userId.slice(0, 12)}...
                      </p>
                      <p className="text-muted-foreground text-xs">
...
function StatCard({
  ...
  return (
    <div className="bg-card rounded-xl border p-4 sm:p-5">
      <div className="flex items-center gap-2.5">
        <div
          className={`inline-flex h-9 w-9 items-center justify-center rounded-lg ${colorMap[color]}`}
        >
...
function MiniStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="border-border/50 bg-card rounded-lg border px-4 py-3">
