  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        if (!url || !ratio) {
          controller.enqueue(
            sendEvent({ type: "error", error: "Missing url or ratio" }),
          );
          controller.close();
          return;
        }
...
Format your response as JSON only, nothing else:
{
  ...
  "colorPalette": ["#hex", "#hex"]
        ...
        const layoutGuidance: Record<string, string> = {
          "1:1":
            "Square layout. Bold title across the top 20%. Central area: 2x2 grid of icon+label cards. Bottom strip: 2-3 stat bubbles side by side. Takeaway quote at the very bottom.",
          "9:16":
            "Tall vertical layout for mobile. Title banner at top. Icons with short labels stacked vertically down the center, each with a unique illustrated icon. Stats in colored pill shapes near the bottom. Takeaway at the base.",
          "16:9":
            "Wide landscape layout. Title on the left 30%. Center: horizontal flow of illustrated icons with labels underneath. Right side: stats in large bold numbers stacked. Takeaway bar along the bottom edge.",
        };
        ...
        const metaphors =
          analysis.visualMetaphors?.length > 0
        ...
        const styleGuides: Record<string, { intro: string; rules: string }> = {
          modern: {
            intro:
              "Design a bold, modern infographic poster. VISUALS FIRST — use large vector illustrations, icons, and graphics. Text should be minimal and punchy.",
...
- NO watermarks, NO attribution text.`,
          ...
          cartoon: {
            intro:
              "Design a fun, cartoon-style infographic poster. Use hand-drawn cartoon illustrations with thick outlines, exaggerated proportions, and a comic-book feel. Bright, saturated, playful colors.",
...
- NO watermarks, NO attribution text.`,
          ...
          minimal: {
            intro:
              "Design a clean, minimalist infographic poster. Emphasis on whitespace, restrained typography, and simple geometric forms. Elegant and understated.",
...
- NO watermarks, NO attribution text.`,
          ...
          lineart: {
            intro:
              "Design an infographic poster using ONLY black ink lines on a pure white background. Pen-and-ink illustration style. No color fills whatsoever.",
...
- NO watermarks, NO attribution text.`,
          ...
          isometric: {
            intro:
              "Design an infographic poster using isometric 3D illustrations. All objects shown from a 30-degree isometric perspective with no vanishing points. Colorful, structured, and technically polished.",
...
- NO watermarks, NO attribution text.`,
          ...
          flat: {
            intro:
              "Design an infographic poster in flat design style. Solid geometric shapes, bold colors, no shadows or gradients. Clean, modern, and grid-aligned.",
...
${activeStyle.rules}`;
        ...
        if (
          rawMessage.includes("token count exceeds") ||
          rawMessage.includes("maximum number of tokens")
        ) {
          userMessage =
            "This video is too long to process. Please try a shorter video (under 45 minutes).";
        } else if (
          rawMessage.includes("Could not resolve") ||
          rawMessage.includes("INVALID_ARGUMENT")
        ) {
          userMessage =
            "Could not access this video. Please check the URL and make sure the video is public.";
        } else if (
          ...
          userMessage =
            "Failed to generate the infographic image. Please try again.";
        } else {
          userMessage =
            "Something went wrong while generating your infographic. Please try again.";
        }
