...
const STYLES: {
...
}[] = [
  {
    value: "modern",
    label: "Modern",
    sublabel: "Bold vectors & gradients",
    icon: Sparkles,
  },
  {
    value: "cartoon",
    label: "Cartoon",
    sublabel: "Playful hand-drawn feel",
    icon: Palette,
  },
  {
    value: "minimal",
    label: "Minimal",
    sublabel: "Clean & understated",
    icon: Minus,
  },
  {
    value: "lineart",
    label: "Line Art",
    sublabel: "Black ink on white",
    icon: PenTool,
  },
  {
    value: "isometric",
    label: "Isometric",
    sublabel: "3D perspective graphics",
    icon: Box,
  },
  {
    value: "flat",
    label: "Flat",
    sublabel: "Solid shapes, no shadows",
    icon: Layers,
  },
];
...
export default function DashboardPage() {
      ...
      if (!response.ok || !response.body) {
        ...
        try {
          ...
          if (
            ...
            parseErr.message !== "Generation failed"
          ) {
            throw parseErr;
          }
  ...
  return (
    <div className="bg-background text-foreground min-h-screen">
      {/* Header */}
      <header className="border-border/50 bg-background/70 border-b backdrop-blur-2xl">
            ...
            {credits !== null && (
              <button
                ...
                className={`flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold transition-all ${
                  credits > 0
                    ? "bg-primary/10 text-primary ring-primary/20 hover:bg-primary/15 ring-1"
                      ...
                      return (
                        ...
                        >
                          <div className="flex w-full items-center">
                            {idx > 0 && (
                              <div
                                className={`h-0.5 flex-1 transition-colors duration-500 ${isCompleted || isActive ? "bg-primary" : "bg-border"}`}
                              />
                            )}
                            <div
                              className={`relative flex h-9 w-9 shrink-0 items-center justify-center rounded-full transition-all duration-500 ${isCompleted ? "bg-primary text-primary-foreground" : ""} ${isActive ? "bg-primary/15 text-primary ring-primary/30 ring-2" : ""} ${!isActive && !isCompleted ? "bg-muted text-muted-foreground" : ""} `}
                            >
                              {isActive && (
                            ...
                            {idx < 2 && (
                              <div
                                className={`h-0.5 flex-1 transition-colors duration-500 ${isCompleted ? "bg-primary" : "bg-border"}`}
                              />
                            )}
                          </div>
                          <span
                            className={`text-xs font-medium transition-colors ${isActive ? "text-primary" : isCompleted ? "text-foreground" : "text-muted-foreground"}`}
                          >
                            {label}
                          </span>
                        </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  <div className="mb-3 flex items-center gap-2.5">
                    <div
                      className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold ${videoId ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
                    >
                  ...
                  <div className="relative">
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter" && !isGenerating && selectedStyle)
                          handleGenerate();
                      }}
                      className="border-border/60 bg-card placeholder:text-muted-foreground/50 focus:border-primary/50 focus:ring-primary/20 w-full rounded-xl border px-5 py-4 text-base shadow-sm transition-all outline-none focus:ring-2"
                    />
                    {error && (
                  ...
                  {videoId && (
                    <div className="border-border/60 bg-card mt-4 overflow-hidden rounded-xl border shadow-sm">
                      <div className="relative aspect-video w-full overflow-hidden bg-black">
                        <img
                ...
                <div
                  className={`transition-all duration-300 ${!videoId ? "pointer-events-none opacity-40" : ""}`}
                >
                  <div className="mb-3 flex items-center gap-2.5">
                    <div
                      className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold ${videoId ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
                    ...
                    {RATIOS.map((ratio) => {
                      ...
                      return (
                        <button
                          ...
                          className={`group relative flex flex-col items-center gap-2 rounded-xl border-2 px-4 py-5 transition-all ${
                            isSelected
                              ? "border-primary bg-primary/[0.06] shadow-primary/10 shadow-md"
                        ...
                        >
                          ...
                          {isSelected && (
                            <div className="ring-primary/30 pointer-events-none absolute -top-px -right-px -bottom-px -left-px rounded-xl ring-2" />
                          )}
                        </button>
                      );
                    })}
                ...
                >
                  <div className="mb-3 flex items-center gap-2.5">
                    <div
                      className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold ${selectedStyle ? "bg-primary text-primary-foreground" : videoId ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground"}`}
                    >
                ...
                {/* Step 4: Generate */}
                <div
                  className={`transition-all duration-300 ${!videoId || !selectedStyle ? "pointer-events-none opacity-40" : ""}`}
                >
                  <div className="mb-3 flex items-center gap-2.5">
                    <div
                      className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold ${videoId && selectedStyle ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground"}`}
                    >
                      4
                    </div>
                  ...
                  {credits !== null && credits <= 0 ? (
                    <Button
                      size="lg"
                      onClick={() => setShowPricing(true)}
                      className="h-12 w-full bg-amber-500 text-base shadow-lg shadow-amber-500/30 transition-all duration-300 hover:bg-amber-600 hover:shadow-xl hover:shadow-amber-500/40"
                    >
                    ...
                    <Button
                      ...
                      className="shadow-primary/30 hover:shadow-primary/40 h-12 w-full text-base shadow-lg transition-all duration-300 hover:shadow-xl disabled:shadow-none"
                    >
            ...
            <div className="mx-auto max-w-2xl">
              <div className="border-border/60 bg-card rounded-2xl border p-4 shadow-sm">
                ...
                <img
                  ...
                  onClick={() => {
                    const match = infographics.find(
                      (i) => i.imageData === previewImage,
                    );
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
              {infographics.map((item) => (
                <div
                  key={item.id}
                  className="group border-border/60 bg-card relative overflow-hidden rounded-xl border shadow-sm transition-all hover:shadow-md"
                >
                  <div
                  ...
                  <div className="p-3">
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      <span className="bg-muted text-muted-foreground rounded-full px-2 py-0.5 font-mono text-[10px] font-medium">
                        {item.ratio}
                      </span>
      ...
      <AlertDialog open={showPricing} onOpenChange={setShowPricing}>
        <AlertDialogContent className="max-h-[85vh] overflow-y-auto overscroll-contain sm:!max-w-2xl [&]:p-4 sm:[&]:p-6">
          <AlertDialogHeader>
          ...
          <div className="grid grid-cols-1 items-stretch gap-3 pt-2 pb-2 sm:grid-cols-3 sm:gap-5 sm:pt-4">
            {PLANS.map((plan) => {
              const Icon = plan.icon;
              return (
                <div
                  key={plan.id}
                  className={`relative flex h-full flex-col overflow-visible rounded-xl border-2 p-3 transition-all sm:p-5 ${
                    plan.popular
                      ? "border-primary bg-primary/[0.04] shadow-primary/10 shadow-md"
                      : "border-border/60 bg-card hover:border-primary/30"
                  } `}
                >
                  {plan.popular && (
                    <div className="absolute -top-3 left-1/2 z-10 -translate-x-1/2">
                      <span className="bg-primary text-primary-foreground inline-flex items-center gap-1 rounded-full px-3 py-0.5 text-[10px] font-bold whitespace-nowrap shadow-sm">
                  ...
                  <div className="flex items-center gap-3 sm:hidden">
                    ...
                    <Button
                      ...
                      className={`h-10 shrink-0 px-4 text-sm font-bold ${plan.popular ? "shadow-primary/20 shadow-md" : ""}`}
                  ...
                  <div className="hidden sm:flex sm:h-full sm:flex-col">
                    ...
                    <Button
                      size="lg"
                      variant={plan.popular ? "default" : "outline"}
                      className={`h-12 w-full text-base font-bold ${plan.popular ? "shadow-primary/20 shadow-md" : ""}`}
                      onClick={() => handleBuyCredits(plan.id)}
                      disabled={buyingPlan === plan.id}
                    >
                      {buyingPlan === plan.id ? (
      ...
      {lightbox && (
        ...
        >
          {/* Top bar */}
          <div className="absolute top-0 right-0 left-0 z-10 flex items-center justify-between px-5 py-4">
            <p className="max-w-[60%] truncate text-sm font-medium text-white/70">
              {lightbox.title}
            </p>
